...
interface ContactHeroImageSettingsProps {
  initialUrl?: string
  siteSettingsMissing?: boolean
}

export function ContactHeroImageSettings({
  initialUrl = '',
  siteSettingsMissing = false,
}: ContactHeroImageSettingsProps) {
  ...
  const handleSave = async () => {
    ...
    const { error: upsertError } = await supabase
      .from('site_settings')
      .upsert(
        { key: 'contact_hero_image_url', value_text: imageUrl || null },
        { onConflict: 'key' }
      )
    ...
    if (upsertError) {
      ...
      if (
        ...
        setError('Tabela site_settings nao encontrada no Supabase. Crie a tabela para salvar a imagem de Contato.')
      } else {
    ...
    setSaving(false)
    setMessage('Imagem da página Contato salva com sucesso.')
  }

  return (
    <section className="bg-white border border-moss/10 p-6 mb-8">
      <div className="mb-5">
        <h3 className="font-serif text-xl text-moss">Imagem de capa (página Contato)</h3>
        <p className="text-sm text-moss/50 mt-1">
          Essa imagem aparece no hero da página <code>/contato</code>.
        </p>
        {siteSettingsMissing && (
